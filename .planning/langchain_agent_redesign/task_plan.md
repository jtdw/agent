...
| Phase 61 CI full-smoke reproducibility | in_progress | Manual `smoke-full` failed on a fresh GitHub runner because ignored `outputs/` Phase45/49/50 evidence was unavailable. Fix in progress on `codex/phase61-fix-full-smoke-ci-evidence`: committed sanitized CI fixtures and rewired manual/nightly full smoke to validate those fixtures while generating the active task window live. |
