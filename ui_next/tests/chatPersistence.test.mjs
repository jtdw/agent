...
assert.match(chatPanelSource, /lastSuccessfulSessionUserIdRef/, 'ChatPanel must remember the last user with valid session data');
assert.match(chatPanelSource, /nextSessions\.length === 0 && lastSuccessfulSessionUserIdRef\.current === requestedUserId/, 'ChatPanel must not let an empty refresh overwrite known sessions for the same user');
