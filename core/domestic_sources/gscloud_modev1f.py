...
from .gscloud_products import MODEV1F_CHINA_250M_EVI_5DAY
...
def parse_modev1f_cells(cells: list[str], row_index: int) -> dict[str, Any] | None:
    ...
    product_tag = "EVI" if ".EVI." in scene_id.upper() or scene_id.upper().endswith(".EVI.V2") else ""
    return {
        "scene_id": scene_id,
        "date": date,
...
def _parse_modev1f_row(row, row_index: int) -> dict[str, Any] | None:
    return parse_modev1f_cells(_row_cells(row), row_index=row_index)


def download_modev1f_china_evi_5day(
    *,
    manager: DataManager,
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(MODEV1F_CHINA_250M_EVI_5DAY.access_url, wait_until="domcontentloaded", timeout=60_000)
            page.wait_for_selector("table, .el-table, .ivu-table, .ant-table, tr", timeout=30_000)
            _try_select_data_available(page)
            page.wait_for_timeout(1200)
            scan = scan_scene_table_pages(page, _parse_modev1f_row, status_path=status_path)
            ...
            if not selected:
                raise RuntimeError(
                    f"未找到满足条件的 MODEV1F EVI 可下载记录。已扫描 {pages_scanned} 页，"
            ...
            for item in selected:
                ...
                goto_scene_page(
                    page,
                    MODEV1F_CHINA_250M_EVI_5DAY.access_url,
    ...
    result["product"] = MODEV1F_CHINA_250M_EVI_5DAY.__dict__
