...
_TOOL_CARDS: tuple[dict[str, Any], ...] = (
    ...
    _card(
        ...
        "从已验证的投影坐标 DEM 生成坡度、坡向、TPI、TRI、DEM-only D8 TWI 等地形派生栅格，可作为土壤水分建模地形因子。",
        ["terrain_analysis", "dem", "slope", "aspect", "tpi", "tri", "twi", "soil_moisture", "dem-only", "d8"],
        ...
        preconditions=["DEM 必须是真实已加载栅格", "DEM 必须有 CRS", "平面坡度和 DEM-only TWI 计算要求投影坐标 CRS", "slope_units 必须明确为 degree 或 percent", "TWI 使用 DEM-only D8 近似，不使用河流距离、up_area 或外部水文数据"],
        common_failure_cases=["DEM 是地理坐标 CRS", "NoData 全覆盖", "derivatives 参数不支持", "输出路径非法", "DEM 洼地或平台区会影响 DEM-only D8 TWI 稳定性"],
        forbidden_uses=["不得直接对经纬度 DEM 计算平面坡度", "不得伪造坡度统计或坡向范围", "不得把 DEM-only D8 TWI 描述为经过填洼、烧河或外部水文校正的结果"],
    ),
    _card(
        "raster_algebra",
